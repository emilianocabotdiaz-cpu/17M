...
const responsablesIniciales = [
  { id: 1, nombre: "Juan Pérez", telefono: "600111222", usuario: "juan" },
  { id: 2, nombre: "María Gómez", telefono: "600333444", usuario: "maria" },
  { id: 3, nombre: "Antonio Ruiz", telefono: "600555666", usuario: "antonio" },
];

const interactoresIniciales = [
  { id: 1, nombre: "Pedro Gómez", telefono: "600777111", usuario: "interactor1", activo: true },
  { id: 2, nombre: "Lucía Romero", telefono: "600777222", usuario: "interactor2", activo: true },
];
...
function Badge({ children, tone = "gray" }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-4 py-1.5 text-xs font-medium ${styles[tone]}`}>{children}</span>;
}
...
function LoginScreen({ onLogin, responsables, interactores }) {
  ...
  const entrar = () => onLogin({ rol, usuario: usuario || "admin" });
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <Card>
          ...
          <div className="mt-5 flex flex-col gap-4 md:flex-row">
            <input value={referencia} onChange={(e) => setReferencia(e.target.value.toUpperCase())} onKeyDown={(e) => e.key === "Enter" && registrar()} placeholder="Referencia" className="h-14 flex-1 rounded-xl border border-slate-200 px-4 text-lg outline-none" />
...
function ResponsableScreen({ onLogout, usuario, vots }) {
...
function CooperativaScreen({ onLogout, vots, setVots, responsables, setResponsables, interactores, setInteractores }) {
  ...
  const crearResponsable = () => {
    ...
    setResponsables([
      ...responsables,
      { id: Date.now(), nombre: nombreResponsable, telefono: telefonoResponsable, usuario },
    ]);
  ...
  const crearInteractor = () => {
    ...
    setInteractores([
      ...interactores,
      { id: Date.now(), nombre: nombreInteractor, telefono: telefonoInteractor, usuario, activo: true },
    ]);
    setNombreInteractor("");
    ...
    <div className="min-h-screen bg-slate-100 px-5 py-6 md:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <Card>
          <div className="flex items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-950">Panel cooperativa</h1>
...
export default function App() {
  ...
  if (sesion.rol === "responsable") {
    return <ResponsableScreen onLogout={() => setSesion(null)} usuario={sesion.usuario} vots={vots} />;
  }
